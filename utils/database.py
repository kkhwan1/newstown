...
def get_connection():
    """데이터베이스 연결 반환"""
    if not DATABASE_URL:
        raise ConnectionError("DATABASE_URL 환경 변수가 설정되지 않았습니다.")
    try:
        return psycopg2.connect(DATABASE_URL)
    except psycopg2.OperationalError as e:
        raise ConnectionError(f"데이터베이스 연결 실패: {e}") from e
...
def init_database() -> bool:
    ...
    try:
        ...
        print(f"[DB ERROR] 데이터베이스 연결 실패: {e}")
        return False
...
def get_news_list(category: Optional[str] = None, status: Optional[str] = None, limit: int = 50, offset: int = 0) -> List[Dict]:
    ...
    try:
        ...
        print(f"[DB ERROR] 뉴스 목록 조회 실패: {e}")
        return []
    finally:
        cur.close()
        conn.close()

def get_news_count(category: Optional[str] = None, status: Optional[str] = None) -> int:
    """뉴스 개수 조회"""
    conn = get_connection()
    cur = conn.cursor()
    ...
    try:
        ...
        print(f"[DB ERROR] 뉴스 개수 조회 실패: {e}")
        return 0
    finally:
        cur.close()
        conn.close()
...
def update_news_status(news_id: int, status: str) -> bool:
    ...
    try:
        ...
        conn.rollback()
        print(f"[DB ERROR] 뉴스 상태 업데이트 실패 (ID: {news_id}): {e}")
        return False
    finally:
        cur.close()
        conn.close()

def save_prompt(name: str, category: str, prompt_text: str) -> Optional[int]:
    """프롬프트 저장
    
    Returns:
        프롬프트 ID 또는 실패 시 None
    """
    conn = get_connection()
    cur = conn.cursor()
...
def get_prompts(category: Optional[str] = None, active_only: bool = True) -> List[Dict]:
    ...
    try:
        ...
        print(f"[DB ERROR] 프롬프트 조회 실패: {e}")
        return []
    finally:
        cur.close()
        conn.close()

def update_prompt(prompt_id: int, name: Optional[str] = None, category: Optional[str] = None, prompt_text: Optional[str] = None, is_active: Optional[bool] = None) -> bool:
    """프롬프트 업데이트
    ...
    try:
        ...
        conn.rollback()
        print(f"[DB ERROR] 프롬프트 업데이트 실패 (ID: {prompt_id}): {e}")
        return False
    finally:
        cur.close()
        conn.close()
...
def delete_prompt(prompt_id: int) -> bool:
    ...
    try:
        ...
        conn.rollback()
        print(f"[DB ERROR] 프롬프트 삭제 실패 (ID: {prompt_id}): {e}")
        return False
    finally:
        cur.close()
        conn.close()
...
def get_news_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        """)
        result = cur.fetchone()
        stats = dict(result) if result else {'total': 0, 'pending': 0, 'uploaded': 0, 'failed': 0}
        ...
        print(f"[DB ERROR] 뉴스 통계 조회 실패: {e}")
        return {'total': 0, 'pending': 0, 'uploaded': 0, 'failed': 0, 'by_category': []}
    finally:
        cur.close()
        conn.close()
