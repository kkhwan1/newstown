...
def validate_category_relevance(title, description, content, target_category):
    """뉴스가 목표 카테고리에 실제로 관련이 있는지 검증 (강화된 필터링)
    
    제목과 본문을 모두 분석하여 해당 카테고리에 맞는 뉴스인지 확인합니다.
    ...
    """
    if target_category not in CATEGORY_KEYWORDS:
        return True, "알 수 없는 카테고리"
    
    title_lower = title.lower()
    full_text = f"{title} {description} {content}".lower()
    
    # 1. 제외 키워드가 제목에 있으면 즉시 제외 (강력한 필터)
    exclude_keywords = CATEGORY_KEYWORDS[target_category].get("exclude", [])
    title_exclude_count = sum(1 for kw in exclude_keywords if kw.lower() in title_lower)
    if title_exclude_count >= 2:
        return False, f"제목에 제외 키워드 {title_exclude_count}개 발견"
    
    # 2. 핵심 키워드가 제목 또는 본문에 있는지 확인
    core_keywords = CATEGORY_KEYWORDS[target_category].get("core", [])
    general_keywords = CATEGORY_KEYWORDS[target_category].get("general", [])
    
    # 제목에서 핵심 키워드 체크
    title_core_count = sum(1 for kw in core_keywords if kw.lower() in title_lower)
    title_general_count = sum(1 for kw in general_keywords if kw.lower() in title_lower)
    
    # 본문에서 키워드 체크
    body_core_count = sum(1 for kw in core_keywords if kw.lower() in full_text)
    body_general_count = sum(1 for kw in general_keywords if kw.lower() in full_text)
    
    # 3. 카테고리별 특별 검증 로직
    if target_category == "연애":
        # 연애 카테고리: 연예와 혼동 방지
        entertainment_keywords = [
            "드라마", "영화", "예능", "방송", "출연", "시청률", "종영", "첫방",
            "복면가왕", "음악", "무대", "라이브", "공연", "콘서트",
            "줄거리", "결말", "캐릭터", "극중", "연기", "촬영"
        ]
        entertainment_count = sum(1 for kw in entertainment_keywords if kw in title_lower)
        if entertainment_count >= 1 and title_core_count == 0:
            return False, f"연예 관련 뉴스로 판단 (연예 키워드 {entertainment_count}개)"
        
        # 연애 프로그램 관련은 허용 (환승연애, 돌싱글즈 등)
        dating_shows = ["환승연애", "돌싱글즈", "나는솔로", "연애의참견", "연애도사"]
        if any(show in title for show in dating_shows):
            return True, "연애 프로그램 관련"
        
        # 제목에 연애 핵심 키워드가 없고 본문에도 적으면 제외
        if title_core_count == 0 and body_core_count < 3:
            return False, f"연애 관련성 낮음 (핵심 키워드: 제목 {title_core_count}, 본문 {body_core_count})"
    
    elif target_category == "스포츠":
        # e스포츠/게임 분리
        esports_keywords = [
            "e스포츠", "이스포츠", "롤드컵", "LCK", "T1", "젠지", "DRX",
            "프로게이머", "게임대회", "배틀그라운드", "발로란트", "오버워치",
            "케스파", "PGC", "크래프톤"
        ]
        esports_count = sum(1 for kw in esports_keywords if kw in full_text)
        if esports_count >= 2:
            return False, f"e스포츠/게임 뉴스로 판단 ({esports_count}개 키워드)"
        
        # 여행/관광 뉴스 제외
        travel_keywords = ["여행", "관광", "투어", "숙박", "호텔", "리조트", "명소"]
        travel_count = sum(1 for kw in travel_keywords if kw in title_lower)
        if travel_count >= 1 and title_core_count == 0:
            return False, f"여행/관광 뉴스로 판단"
        
        # 건강/의료 뉴스 제외 (선수 부상 관련 제외)
        health_keywords = ["병원", "진료", "치료", "수술", "건강", "질환", "증상", "예방"]
        health_count = sum(1 for kw in health_keywords if kw in title_lower)
        if health_count >= 2:
            return False, f"건강/의료 뉴스로 판단"
        
        # 스포츠 핵심 키워드 체크
        teams = CATEGORY_KEYWORDS["스포츠"].get("teams", [])
        players = CATEGORY_KEYWORDS["스포츠"].get("famous_players", [])
        has_team = any(team in full_text for team in teams)
        has_player = any(player in full_text for player in players)
        
        if title_core_count == 0 and not has_team and not has_player and body_core_count < 3:
            return False, f"스포츠 관련성 낮음 (핵심 키워드 부족)"
    
    elif target_category == "경제":
        # 경제 뉴스 검증은 비교적 느슨하게 (다양한 주제 포함)
        if title_core_count == 0 and body_core_count < 2:
            return False, f"경제 관련성 낮음"
...
def extract_proper_nouns(text):
    """고유명사(인물명, 기관명 등) 추출 - 중복 체크 강화용"""
    if not text:
        return set()
    
    proper_nouns = set()
    
    # 1. 따옴표 안의 인물명/작품명 추출 ('아이유', "손흥민" 등)
    quoted = re.findall(r"['\"]([가-힣a-zA-Z]{2,10})['\"]", text)
    for q in quoted:
        if len(q) >= 2:
            proper_nouns.add(q)
    
    # 2. 한글 2-4글자 이름 패턴 (성+이름)
    # 일반적인 한국인 이름 패턴
    name_pattern = re.findall(r'([가-힣]{2,4})(?:\s*씨|\s*선수|\s*감독|\s*대표|\s*의원|\s*장관|\s*사장|\s*회장|\s*아나운서|\s*기자)', text)
    for name in name_pattern:
        proper_nouns.add(name)
    
    # 3. '~가', '~은', '~이' 앞의 2-3글자 이름 (주어 위치)
    subject_names = re.findall(r'([가-힣]{2,3})(?:가|은|이|의|와|과)\s', text)
    for name in subject_names:
        # 일반 명사 제외
        common_words = {'오늘', '내일', '어제', '뉴스', '기사', '사진', '영상', '관련', '최근', '현재', '앞서', '이후', '경기', '대회'}
        if name not in common_words and len(name) >= 2:
            proper_nouns.add(name)
    
    # 4. 드라마/프로그램 제목 패턴
    drama_pattern = re.findall(r"['\"]([가-힣a-zA-Z0-9\s]{2,20})['\"]", text)
    for title in drama_pattern:
        title = title.strip()
        if 2 <= len(title) <= 15:
            proper_nouns.add(title)
    
    # 5. 알파벳+숫자 조합 (예: AAA, MVP, KBO 등)
...
def check_duplicate_in_cache(existing_data, link, title=None, content=None):
    ...
    if title and title.strip():
        ...
        for i, (existing_title, normalized_existing_title) in enumerate(zip(existing_data['titles'], existing_data['normalized_titles'])):
            ...
            if is_same_topic(title, content, existing_title, ex_content):
                return True
...
def main():
    ...
    if news_data and 'items' in news_data:
        ...
        for result in news_results:
            ...
            is_valid, reason = validate_category_relevance(title, description, content, category)
            if not is_valid:
                print(f"      [필터링] {title[:30]}... - {reason}")
        ...
        if skipped_validation_count > 0:
            print(f"[SKIP] 카테고리 검증 실패로 필터링된 뉴스: {skipped_validation_count}개")
